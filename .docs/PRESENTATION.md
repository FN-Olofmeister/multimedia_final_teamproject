...
> "사진을 작게 만들면 얼마나 화질이 나빠지는지 측정하는 기능입니다!"

```
🔄 작동 원리 (3단계)

1️⃣ 웹캠에서 사진 한 장 캡처 (PNG - 무손실 원본)
   └─ 예: 2MB 크기의 선명한 사진

2️⃣ 서버에서 품질 조절해서 압축 (JPEG)
   └─ 품질 70% → 200KB로 줄어듦

3️⃣ 원본 vs 압축본 비교해서 점수 계산
   └─ PSNR: 35dB (높을수록 좋음, 30 이상이면 OK)
   └─ SSIM: 0.95 (1에 가까울수록 원본과 똑같음)
```

**💡 비유로 이해하기:**
- **PSNR**: 시력 검사표에서 "얼마나 선명하게 보이나요?" 측정
- **SSIM**: "원본 사진과 얼마나 비슷해 보이나요?" 측정
...
### 4.1.1 📁 파일 전송 압축 (이미지/영상/오디오)

**📊 일반인을 위한 쉬운 설명:**

> "파일을 보내기 전에 크기를 줄여서 빠르게 전송하는 기능입니다!"

```
🖼️ 이미지 압축 흐름

[원본 사진 선택] → [품질 슬라이더 70%] → [JPEG로 압축] → [전송]
     2MB              조절 가능            400KB         빠름!
                                        (80% 감소)
```
...
[원본 영상 선택] → [품질 슬라이더 70%] → [서버에서 H.264 압축] → [전송]
    100MB              조절 가능              30MB              빠름!
                                          (70% 감소)
```
...
[원본 음악 선택] → [품질 슬라이더] → [서버에서 MP3 압축] → [전송]
    50MB           80%=320kbps            10MB          빠름!
                   60%=192kbps
                   40%=128kbps
```
...
| 슬라이더 | 이미지 | 영상 | 오디오 |
|---------|--------|------|--------|
| 100% | 거의 원본 | 최고 화질 | 320kbps (CD급) |
| 70% (권장) | 눈에 차이 없음 | 좋은 화질 | 192kbps |
| 30% | 살짝 뭉개짐 | 화질 저하 | 96kbps (라디오급) |
